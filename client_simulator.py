...
CCTV yang mengirim status kepadatan, ke sebuah Edge Node lewat socket TCP.
...
edge_node.py & main_server.py benar-benar bekerja end-to-end.
...
Cara menjalankan:
    ...
    python client_simulator.py --port 9001 --region Jaksel --jumlah 100 --chaos
"""
...
async def main(host: str, port: int, region: str, jumlah: int, chaos: bool):
    ...
    print(f"[Client] mengirim {jumlah} data dummy (campuran laporan warga & sensor CCTV) "
          f"ke EdgeNode-{region} di {host}:{port}{mode_str} ...\n")
